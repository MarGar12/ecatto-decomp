...
func _process(delta):
	#debug stuff
	get_window().title = "Element Cattos Decomp on " + str(OS.get_name()) + " || " + str(Engine.get_frames_per_second()).pad_decimals(0) + " FPS " + str(Engine.get_frames_drawn()).pad_decimals(0)
